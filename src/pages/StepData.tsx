...
import {
  getAvailableDates, calcCheckOut, PERIOD_SLOTS,
  TYPE_HOURS, isWeekendDate, defaultTypeFor,
} from '../data'
import { useStore } from '../store/useStore'
import { fetchPmsDayMap, type SlotAvailability } from '../lib/plugplay'
...
export default function StepData() {
  ...
  // Modalidade que vale para o cálculo de disponibilidade. O StepData vem
  // antes do StepTipo em parte dos fluxos, então `type` pode estar vazio — e
  // em fim de semana o padrão é Pernoite, já que Período não é vendido.
  const effectiveType = type ?? (selectedDate ? defaultTypeFor(selectedDate) : 'period')

  // ── Disponibilidade real do PMS, por horário ──────────────
  // O resultado é carimbado com a chave (data + modalidade) que o originou.
  // Assim `pmsSlots` e `pmsLoading` saem por derivação e o efeito só escreve
  // state no callback assíncrono — sem reset síncrono, sem render em cascata.
  const pmsKey = selectedDate ? `${selectedDate.toDateString()}|${effectiveType}` : null
  const [pmsData, setPmsData] = useState<
    { key: string; slots: Record<string, SlotAvailability> | null } | null
  >(null)

  useEffect(() => {
    if (!selectedDate || !pmsKey) return
    let cancelled = false
    fetchPmsDayMap(selectedDate, TYPE_HOURS[effectiveType], PERIOD_SLOTS)
      .then(r => { if (!cancelled) setPmsData({ key: pmsKey, slots: r?.slots ?? null }) })
    return () => { cancelled = true }
  // selectedDate e effectiveType já estão codificados em pmsKey
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pmsKey])

  const pmsLoaded  = !!pmsKey && pmsData?.key === pmsKey
  const pmsSlots   = pmsLoaded ? pmsData!.slots : null
  const pmsLoading = !!pmsKey && !pmsLoaded

  /**
   * Suítes livres no horário, ou null quando não há informação do PMS.
   *
   * null e 0 são coisas diferentes: sem dado o slot segue clicável (o
   * create-charge confere antes de cobrar), esgotado ele bloqueia.
   */
  function slotFree(slot: string): number | null {
    return pmsSlots?.[slot]?.available ?? null
  }

  // Todos os horários do dia selecionado já passaram?
  const allSlotsPast = isToday && slots.every(slot => isSlotPast(slot))

  // Dia inteiro esgotado no PMS (só afirma isso com dado na mão)
  const allSlotsFull = !!pmsSlots && !allSlotsPast &&
    slots.every(slot => isSlotPast(slot) || slotFree(slot) === 0)

  // Próxima data disponível após a selecionada (para o atalho no aviso)
  const nextAvailableDate = (allSlotsPast || allSlotsFull) && selectedDate
    ? (AVAILABLE_DATES.find(d => d > selectedDate) ?? null)
    : null

  // O slot escolhido pode esgotar entre a escolha e a chegada do dado do PMS.
  // Derivamos em vez de limpar o state num efeito: evita render em cascata e
  // mantém a escolha do usuário se a suíte voltar a ficar livre.
  const activeSlot = selectedSlot && !isSlotPast(selectedSlot) && slotFree(selectedSlot) !== 0
    ? selectedSlot
    : null
  ...
  const selectedCheckOut =
    selectedDate && activeSlot
      ? calcCheckOut(slotCheckIn(activeSlot, selectedDate), effectiveType)
      : null

  function confirm() {
    if (!selectedDate || !activeSlot) return
    setCheckIn(slotCheckIn(activeSlot, selectedDate))
    nextStep()
  }

  const canContinue = !!(selectedDate && activeSlot)
          ...
          {selectedDate && (
            <div ref={slotsRef} className="mb-8" style={{ scrollMarginTop: '5rem' }}>
              <div className="flex items-baseline justify-between mb-3 gap-3">
                ...
                {pmsLoading ? (
                  <span className="text-[10px] text-gold-700/60 animate-pulse">
                    verificando disponibilidade…
                  </span>
                ) : pmsSlots ? (
                  <span className="text-[10px] text-gold-700/50">disponibilidade em tempo real</span>
                ) : null}
              </div>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                {slots.map((slot) => {
                  ...
                  const free = slotFree(slot)
                  const full = free === 0
                  const disabled = past || full

                  // Legenda: horário passado > esgotado > quantas restam > turno
                  const legend = past
                    ? 'indisponível'
                    : full
                    ? 'esgotado'
                    : free !== null && free <= 3
                    ? `${free} ${free === 1 ? 'suíte' : 'suítes'}`
                    : period
                  ...
                  return (
                    ...
                    >
                      <span className={disabled ? 'line-through' : ''}>{slot}</span>
                      <span className={[
              ...
              {/* Aviso: todas as suítes ocupadas neste dia, segundo o PMS */}
              {allSlotsFull && (
                <div
                  className="mt-4 flex items-start gap-3 px-4 py-3 rounded-xl"
                  style={{ background: 'rgba(201,168,76,0.04)', border: '1px solid rgba(201,168,76,0.14)' }}
                >
                  ...
                  <p className="text-[11px] leading-relaxed" style={{ color: 'rgba(210,195,165,0.55)' }}>
                    Todas as suítes já estão ocupadas nesta data.{' '}
                    {nextAvailableDate ? (
                      <>
                        Tente{' '}
                        <button
              ...
              {selectedCheckOut && activeSlot && (
                <div className="mt-4 px-4 py-3 rounded-xl border border-gold-800/30 bg-gold-900/10">
                  ...
                  <div className="flex items-center gap-3 text-sm flex-wrap">
                    <span className="text-gold-400 font-medium">Check-in: {activeSlot}</span>

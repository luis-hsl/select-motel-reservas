...
function waLinkRecovery(lead: Lead) {
  const digits = lead.phone.replace(/\D/g, '')
  const num = digits.startsWith('55') ? digits : `55${digits}`
  const firstName = lead.name.split(' ')[0] || 'você'

  const parts: string[] = [`Olá, ${firstName}! Aqui é o Select Motel. 😊`]

  const ctx: string[] = []
  if (lead.package_id) ctx.push(`Pacote ${capitalize(lead.package_id)}`)
  if (lead.type) ctx.push(lead.type === 'period' ? 'período' : 'pernoite')
  if (lead.suite_id) ctx.push(`suíte ${lead.suite_id.replace('suite-', '')}`)

  if (ctx.length > 0) {
    parts.push(`Vi que você estava reservando ${ctx.join(' · ')}, mas não chegou ao pagamento.`)
  } else {
    parts.push(`Vi que você iniciou uma reserva mas não chegou ao pagamento.`)
  }

  if (lead.check_in) {
    const d = new Date(lead.check_in)
    parts.push(`Data pretendida: ${d.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}.`)
  }
    ...
    return leads.filter(l => {
      ...
      if (search) {
        ...
        return (
          ...
          (qDigits.length > 0 && (l.tax_id ?? '').replace(/\D/g, '').includes(qDigits)) ||
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <input
          type="text"
          placeholder="Nome, telefone, e-mail, CPF, campanha..."
          value={search}
            ...
            <div key={l.id} className="bg-white/[0.03] border border-white/8 rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {l.mode && (
                      <span className={[
                        ...
                        {l.mode === 'package' ? 'Pacote' : 'Experiência'}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex flex-col gap-1">
                  <a
                    href={waLinkRecovery(l)}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={`Abrir WhatsApp de ${l.name} com mensagem de recuperação`}
                    title="Mensagem de recuperação pré-gerada"
                  ...
                  {l.whatsapp_consent ? (
                    <span className="text-[10px] text-green-400/70 pl-0.5">✓ Autorizou receber mensagens</span>
                  ) : (
                    <span className="text-[10px] text-white/25 pl-0.5">Não autorizou mensagens</span>
                  )}
                </div>
                <span className="text-white/35 text-xs truncate">{l.email}</span>
                {l.tax_id && (
                  <span className="text-white/30 text-[11px] tabular-nums" title="CPF">
                    {fmtCPF(l.tax_id)}
                  </span>
                )}
              </div>

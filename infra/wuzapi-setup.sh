...
echo
echo "    1b) Apontar o webhook de entrada (vale tanto pra instancia nova"
echo "        quanto pra uma que ja esta pareada — nao derruba a sessao):"
echo "       curl -X POST http://127.0.0.1:8080/webhook \\"
echo "            -H 'token: SEU_USER_TOKEN' -H 'Content-Type: application/json' \\"
echo "            -d '{\"webhookurl\":\"${WUZAPI_GLOBAL_WEBHOOK}\",\"events\":[\"Message\"]}'"
echo
echo "        ATENCAO: o campo e 'webhookurl'. O POST aceita 'webhook' sem"
echo "        reclamar, devolve 200 e ignora a URL — so os eventos aplicam."
echo "        Confira sempre com: curl http://127.0.0.1:8080/webhook -H 'token: ...'"
echo
